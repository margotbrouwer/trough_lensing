...
thetalist = np.array([5., 10., 15., 20., 6.835])/60.
#thetalist = np.array([5., 10., 6.835])/60.
...
for ij in range(1):
    ...
    if 'highZ' in selection:
        ...
        #thetalist = np.array([3.417, 6.835, 10.25, 13.67])/60.
    ...
    # Density
    if 'Z' in selection:
        troughDc = (cosmo.comoving_distance(redshift_av).to('Mpc')).value
        troughDa = troughDc / (1+redshift_av)
        if len(thetalist) > 1:
            distances = np.reshape([troughDc, troughDa], [2,1])
        else:
            distances = troughDa
        
        am_to_rad = np.pi/(60.*180.)
        
        Aefftheta_tot = (Nmasktheta_tot/mask_density) * (am_to_rad * distances)**2. # Effective area of each circle (in Mpc^2)
